...
/// Search for accounts
pub async fn search_accounts(configuration: &configuration::Configuration, query: &str, field: crate::models::AccountSearchFieldFilter, page: Option<i32>, _type: Option<crate::models::AccountTypeFilter>) -> Result<crate::models::AccountArray, Error<SearchAccountsError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/api/v1/search/accounts", local_var_configuration.base_path);
...
pub async fn search_transactions(configuration: &configuration::Configuration, query: &str, page: Option<i32>) -> Result<crate::models::TransactionArray, Error<SearchTransactionsError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/api/v1/search/transactions", local_var_configuration.base_path);
    let mut local_var_req_builder = local_var_client.request(reqwest::Method::GET, local_var_uri_str.as_str());
